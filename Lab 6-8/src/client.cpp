...
int ConvertStrToNum(const std::string& commString) {
    if (commString == "start") return 0;
    if (commString == "time") return 1;
    return 2;
}  
...
async_node* find_node_exec(async_node* ptr, int id) {
    ...
    if (ptr->id > id) return find_node_exec(ptr->L, id);
    if (ptr->id < id) return find_node_exec(ptr->R, id);
    return ptr;
}
...
async_node* find_node_create(async_node* ptr, int id) {
    if (ptr == nullptr) return nullptr;
    if (ptr->L == nullptr && ptr->id > id) return ptr;
    if (ptr->R == nullptr && ptr->id < id) return ptr;
    if (ptr->id > id) return find_node_create(ptr->L, id);
    if (ptr->id < id) return find_node_create(ptr->R, id);
    return nullptr;
}
...
bool destroy_node(async_node*& ptr, int id) {
    ...
    if (ptr->id > id) return destroy_node(ptr->L, id);
    if (ptr->id < id) return destroy_node(ptr->R, id);
    ptr->active = false;
    ptr->make_query({REMOVE});
    if (ptr->L != nullptr) destroy_node(ptr->L, ptr->L->id);
    if (ptr->R != nullptr) destroy_node(ptr->R, ptr->R->id);
    ptr = nullptr;
    return true;
}
